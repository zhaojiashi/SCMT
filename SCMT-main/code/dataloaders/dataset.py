...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if 'onehot_label' in sample:
            for i in range(0, img_s2.shape[2]):
                temp = img_s2[:, :, i]
                temp = Image.fromarray((temp * 255).astype(np.uint8))
                # if random.random() < 0.8:
                #     temp = transforms.ColorJitter(0.5, 0.5, 0.5, 0.25)(temp)
                    # temp = transforms.RandomGrayscale(p=0.2)(temp)
                temp = blur(temp, p=0.5)
                temp = torch.from_numpy(np.array(temp)).float() / 255.0
                img_s2[:, :, i] = temp

            img_s2 = img_s2.reshape(1, img_s2.shape[0], img_s2.shape[1], img_s2.shape[2]).astype(np.float32)
            cutmix_box1 = obtain_cutmix_box(image.shape[1], p=0.5)
            return {'image': torch.from_numpy(image), 'label': torch.from_numpy(sample['label']).long(),
                    'img_s': torch.from_numpy(img_s2), 'cutmix': cutmix_box1}


        else:
            for i in range(0,img_s2.shape[2]):
                temp = img_s2[:,:,i]
                temp = Image.fromarray((temp * 255.0).astype(np.uint8))
                # if random.random() < 0.8:
                #     temp = transforms.ColorJitter(0.5, 0.5, 0.5, 0.25)(temp)
                    # temp = transforms.RandomGrayscale(p=0.2)(temp)
                temp = blur(temp, p=0.5)
            ...
            img_s2 = img_s2.reshape(1, img_s2.shape[0], img_s2.shape[1], img_s2.shape[2]).astype(np.float32)
            cutmix_box1 = obtain_cutmix_box(image.shape[1], p=0.5)
            return {'image': torch.from_numpy(image), 'label': torch.from_numpy(sample['label']).long(),'img_s':torch.from_numpy(img_s2),'cutmix':cutmix_box1}
